<?php
  required_once'head.php'
  required_once'menu.php'
  ?>

    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <a class="navbar-brand" href="#">Academia</a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#conteudoNavbarSuportado" aria-controls="conteudoNavbarSuportado" aria-expanded="false" aria-label="Alterna navegação">
          <span class="navbar-toggler-icon"></span>
        </button>
      
        <div class="collapse navbar-collapse" id="conteudoNavbarSuportado">
          <ul class="navbar-nav mr-auto">
            <li class="nav-item active">
              <a class="nav-link" href="#">Home <span class="sr-only">(página atual)</span></a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="#">Nossa Academia</a>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                Atividades
              </a>
              <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                <a class="dropdown-item" href="#">Jumpp</a>
                <a class="dropdown-item" href="#">HidroJumpp</a>
                <a class="dropdown-item" href="#">Danças</a>
                
              </div>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                Loja
              </a>
              <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                <a class="dropdown-item" href="Roupas.html">Roupas</a>
                <a class="dropdown-item" href="Suplementos.html">Suplementos</a>
                <a class="dropdown-item" href="Diversos.html">Diversos</a>
                
              </div>
            </li>
      </nav>
''
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-8 text-center">
                <h2>Diversos</h2>
            </div>
        </div>
    </div>
    <div class="container-fluid imagens">
        <div class="row">
            <div class="col-md-4">
                <img src="imagens/diversos01.jpg" class="img-fluid">
            </div>

            <div class="col-md-4">
                <img src="imagens/diversos02.jpg" class="img-fluid">
            </div>

            <div class="col-md-4">
                <img src="imagens/diversos03.jpg" class="img-fluid">
            </div>

        </div>
    </div>

    <?php
    require_once 'footer.php'
    ?>
